# Feature Selection for Industrial Sensor Anomaly Detection
...
**Main Finding**: Feature selection maintains or slightly improves model performance while significantly reducing feature dimensionality.---
...
Importance(f) = Σ p(t) × Δi(t,f)

Dimana:
- p(t) = Proportion of samples reaching node t
- Δi(t,f) = Decrease in impurity by splitting on feature f
...
- SWAT: 40% feature reduction with +0.36% F1 change
- WADI: 86% feature reduction with +0.33% F1 change
- All tested models maintain baseline performance
- Training speedup: 1.05x - 3.14x across models

**Observation**: 
Feature selection does not significantly degrade model performance. The slight improvements observed may be due to reduced overfitting from removing redundant features.
...
**Explanation**:
1. SWAT features are treatment-specific (chemical dosing, settling tanks)
2. WADI features are distribution-specific (leak detection, network pressure)
3. Different installations have unique sensor configurations
4. Feature selection adapts to each system's architecture
...
#### **For SWAT Dataset**:
1. **Recommended Configuration**: Threshold ≥6 (21 features)
   - F1-Score: 0.9733
   - Reduction: 59.6% (from 52 to 21 features)
   - Training: 1.3x - 3.1x faster
   - Model: Random Forest shows best results

2. **High-Confidence Features** (9/9 votes - 8 features):
   - These features have unanimous agreement across methods
   - Can be prioritized for real-time monitoring
   - Highest confidence based on ensemble voting
...
1. **Site-Specific Deployment**:
   - Zero common features indicates each site needs custom selection
   - This is expected given different system architectures
   - Run feature selection pipeline for each deployment site

2. **Methodology Transfer**:
   - Apply same methodology (not specific features) to new sites
   - Use same pipeline: 9 methods with voting mechanism
   - Adjust thresholds based on site-specific characteristics

3. **Monitoring Strategy**:
   - SWAT: Prioritize 8 high-confidence features (9/9 votes)
   - WADI: Monitor all 17 selected features
   - Both: Apply threshold optimization based on resources
...
- Multiple models tested (Random Forest, XGBoost, LightGBM)
- Performance maintained with slight improvements (+0.33% to +2.0% F1)
- Reduced sensitivity to individual method variations
- Vote counts provide feature confidence interpretation
- Threshold selection optimized via F1-Score
- Training time reduced by 1.1x - 3.1x
- Validation completed across multiple algorithms
...
- Documentation for academic or industry use

### Key Outcomes:

**Validated Performance**: 
- Feature selection maintains baseline accuracy with slight improvements
- No significant performance degradation despite feature reduction

**Implementation Status**:
- Recommendations based on F1-Score optimization
- Training speedup demonstrated across multiple models
- Comprehensive validation completed

**Documentation Quality**:
- Statistical validation with multiple model testing
- Transparent methodology with voting mechanisms
- Complete documentation and reproducibility support

**Research Contributions**:
- Cross-dataset behavior analysis (site-specific selection)
...
**Project**: Data Mining UAS Implementation  
**Date**: November 2024  
**Version**: 3.0 Enhanced (Ensemble Feature Selection with SMOTE and Validation)
