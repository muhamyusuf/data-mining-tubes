"""
================================================================================
DATASET 1: COMPARISON OF FEATURE SELECTION METHODS
================================================================================
Tugas Besar Penambangan Data 2025

OBJECTIVE:
    Membandingkan 4 metode feature selection untuk prediksi demand farmasi:
...
print("=" * 80)
print("DATASET 1: COMPARISON OF FEATURE SELECTION METHODS")
print("=" * 80)
...
# ============================================================================
# 1. DATA LOADING & PREPROCESSING (SAMA SEPERTI SEBELUMNYA)
# ============================================================================
...
print(f"✅ Loaded {len(df_agg):,} samples")
...
print(f"✅ Created {len(feature_cols)} features from {len(df_agg):,} samples")
...
def evaluate_models(X_tr, X_te, y_tr, y_te, method_name):
    ...
    # Decision Tree
    dt = DecisionTreeClassifier(max_depth=8, min_samples_split=50, random_state=42)
    dt.fit(X_tr, y_tr)
...
# Jumlah fitur yang akan dipilih (konsisten untuk semua metode filter)
K_FEATURES = 10
...
# Mutual info works with scaled data
...
# Chi-square requires non-negative features, so we need to scale to [0, 1]
...
print(f"   Top 5: {selected_features_chi[:5]}")
...
for i, col in enumerate(feature_cols):
    ...
    correlations.append((i, abs(corr)))  # Use absolute value
...
print(f"   Top 5: {selected_features_rfecv[:5]}")
...
df_results.to_csv(output_dir / 'dataset1_feature_selection_comparison.csv', index=False)
print(f"\n✅ Saved: {output_dir / 'dataset1_feature_selection_comparison.csv'}")
...
fig.suptitle('Dataset 1: Perbandingan Metode Feature Selection', fontsize=16, fontweight='bold')
...
pivot_data = pivot_data.reindex(methods)  # Order by methods list
sns.heatmap(pivot_data, annot=True, fmt='.4f', cmap='RdYlGn', cbar_kws={'label': 'F1-Score'},
            linewidths=0.5, ax=ax4, vmin=0.75, vmax=1.0)
ax4.set_title('F1-Score Heatmap', fontweight='bold')
ax4.set_xlabel('Model', fontweight='bold')
ax4.set_ylabel('Method', fontweight='bold')

plt.tight_layout()
plt.savefig(output_dir / 'dataset1_feature_selection_comparison.png', dpi=300, bbox_inches='tight')
print(f"✅ Saved: {output_dir / 'dataset1_feature_selection_comparison.png'}")
