...
# NEW: Exponentially weighted moving average (EWMA) for recent trend
df_features['qty_ewma_7'] = df_features.groupby('kd_obat')['qty_total'].shift(1).ewm(span=7, adjust=False).mean()

# NEW: Change from previous day (growth rate)
df_features['qty_change_1'] = df_features['qty_lag_1'] - df_features['qty_lag_2']

# NEW: Coefficient of variation (normalized volatility)
df_features['qty_cv_7'] = df_features[f'qty_roll_std_7'] / (df_features[f'qty_roll_mean_7'] + 1)
...
print(f"   Target range: min={y.min():.2f}, max={y.max():.2f}, mean={y.mean():.2f}, median={y.median():.2f}")
...
# Measure inference time (100 iterations for stable measurement)
start_time = time.time()
for _ in range(100):
    _ = model_baseline.predict(X_test_scaled[:10])  # Sample batch
baseline_inference_time = (time.time() - start_time) / 100
...
rfecv_total_time = rfecv_selection_time + rfecv_train_time

# Measure inference time (100 iterations for stable measurement)
start_time = time.time()
for _ in range(100):
    _ = model_rfecv.predict(X_test_scaled[selected_rfecv][:10])  # Sample batch
rfecv_inference_time = (time.time() - start_time) / 100
...
print(f"   RMSE: {rmse_rfecv:.2f} | MAE: {mae_rfecv:.2f} | R²: {r2_rfecv:.4f}")
print(f"   Selection time: {rfecv_selection_time:.4f}s | Training time: {rfecv_train_time:.4f}s | Total: {rfecv_total_time:.4f}s")
print(f"   Selection time: {rfecv_selection_time:.4f}s | Training time: {rfecv_train_time:.4f}s | Total: {rfecv_total_time:.4f}s")
...
kbest_selection_time = time.time() - start_time
...
kbest_train_time = time.time() - start_time
kbest_total_time = kbest_selection_time + kbest_train_time

# Measure inference time (100 iterations for stable measurement)
start_time = time.time()
for _ in range(100):
    _ = model_kbest.predict(X_test_scaled[selected_kbest][:10])  # Sample batch
kbest_inference_time = (time.time() - start_time) / 100
...
print(f"   RMSE: {rmse_kbest:.2f} | MAE: {mae_kbest:.2f} | R²: {r2_kbest:.4f}")
print(f"   Selection time: {kbest_selection_time:.4f}s | Training time: {kbest_train_time:.4f}s | Total: {kbest_total_time:.4f}s")
...
results = pd.DataFrame({
    ...
    'Selection_Time_s': [0.0, rfecv_selection_time, kbest_selection_time],
    'Training_Time_s': [baseline_train_time, rfecv_train_time, kbest_train_time],
    'Total_Time_s': [baseline_train_time, rfecv_total_time, kbest_total_time],
    'Inference_Time_ms': [baseline_inference_time*1000, rfecv_inference_time*1000, kbest_inference_time*1000]
})
...
print(f"\nTraining Efficiency:")
print(f"  Baseline:    Training = {baseline_train_time:.4f}s | Total = {baseline_train_time:.4f}s")
print(f"  RFECV:       Selection = {rfecv_selection_time:.4f}s | Training = {rfecv_train_time:.4f}s | Total = {rfecv_total_time:.4f}s")
print(f"               Training speedup: {(baseline_train_time/rfecv_train_time):.2f}x {'✅ FASTER' if rfecv_train_time < baseline_train_time else '❌ SLOWER'}")
print(f"  SelectKBest: Selection = {kbest_selection_time:.4f}s | Training = {kbest_train_time:.4f}s | Total = {kbest_total_time:.4f}s")
print(f"               Training speedup: {(baseline_train_time/kbest_train_time):.2f}x {'✅ FASTER' if kbest_train_time < baseline_train_time else '❌ SLOWER'}")

print(f"\nInference Efficiency (Production-Critical):")
print(f"  Baseline:    {baseline_inference_time*1000:.4f}ms per batch")
print(f"  RFECV:       {rfecv_inference_time*1000:.4f}ms per batch | Speedup: {(baseline_inference_time/rfecv_inference_time):.2f}x {'✅ FASTER' if rfecv_inference_time < baseline_inference_time else '❌ SLOWER'}")
print(f"  SelectKBest: {kbest_inference_time*1000:.4f}ms per batch | Speedup: {(baseline_inference_time/kbest_inference_time):.2f}x {'✅ FASTER' if kbest_inference_time < baseline_inference_time else '❌ SLOWER'}")
